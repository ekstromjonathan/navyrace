...
3. **Journal is truth** — Facts/tracks/entries/notes are durable; `message_log` is short working memory. When OpenRouter fails, answer the actual question from the journal (week shape, yesterday vs today, what was logged) — never dump today’s workout as a fake reply. If they already answered (bytte / rolig dag) do that, don’t repeat the offer. If the model is down, say so in one line *and* still give a useful answer.
